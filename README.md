...
The script creates user `elv-alc` (`dialout`), installs to `/opt/alc`, builds the frontend, installs udev + unit, then `enable --now`. UI: `http://<IP>:8080` (listens on `0.0.0.0:8080`).
